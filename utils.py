...
class CustomizedBartEncoder(BartEncoder):
    def __init__(self, config, embed_tokens):
        super().__init__(config, embed_tokens)
        print(f"Use Customized Bart Encoder !!!", flush=True)
    ...
    def forward(
        ...
        if input_ids is not None and inputs_embeds is not None:
            ...
            # input: for positional embedding
            input = input_ids
...
class CustomizedBartClassificationHead(nn.Module):
    def __init__(
        self, input_dim, inner_dim, num_classes,
    ):
        print(f"\nInitialized Customized Bart Classification Head !!!\n", flush=True)
        super().__init__()
...
class MultiWozReader(_ReaderBase):
    def __init__(self, vocab=None, args=None):
        ...
        self.args = args
        print(f"\nTrain set fraction: {self.args.fraction}; size: {round(self.args.fraction * 8438)}\n", flush=True)
