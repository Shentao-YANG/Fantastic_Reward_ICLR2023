import json, argparse, os, re, copy, zipfile
...
def get_db_values(value_set_path, data_folder):
    processed = {}
    bspn_word = []
    ...
    with open(f'{data_folder}/multi-woz-processed/bspn_word_collection.json', 'w') as f:
...
class DataPreprocessor(object):
    def __init__(self):
        ...
        data_path = f'{cfg.data_folder}/multi-woz/annotated_user_da_with_span_full.json'
        archive = zipfile.ZipFile(data_path + '.zip', 'r')
        self.convlab_data = json.loads(archive.open(data_path.split('/')[-1], 'r').read().lower())
        self.delex_sg_valdict_path = f'{cfg.data_folder}/multi-woz-processed/delex_single_valdict.json'
        self.delex_mt_valdict_path = f'{cfg.data_folder}/multi-woz-processed/delex_multi_valdict.json'
        self.ambiguous_val_path = f'{cfg.data_folder}/multi-woz-processed/ambiguous_values.json'
        self.delex_refs_path = f'{cfg.data_folder}/multi-woz-processed/reference_no.json'
    ...
    def preprocess_main(self, save_path=None, is_test=False):
        ...
        for fn, raw_dial in tqdm(list(self.convlab_data.items())):
            ...
            data[fn] = dial
            # pprint(dial)
            if cfg.test_mode and (count == 20):     # create a smaller dataset for testing
                break
        self.vocab.construct()
        self.vocab.save_vocab(f'{cfg.data_folder}/multi-woz-processed/vocab')
        with open(f'{cfg.data_folder}/multi-woz-analysis/dialog_acts.json', 'w') as f:
            json.dump(ordered_sysact_dict, f, indent=2)
        with open(f'{cfg.data_folder}/multi-woz-analysis/dialog_act_type.json', 'w') as f:
...
if __name__=='__main__':
    ...
    parser.add_argument("--test_mode", type=int, default=0)     # default(0) do not use test_mode
    parser.add_argument("--exp_idx", type=str, default="0")
    args = parser.parse_args()
    assert args.data_folder != ""
    assert args.test_mode in (0, 1)

    cfg.data_folder = args.data_folder
    cfg.test_mode = args.test_mode == 1
    ...
    get_db_values('db/value_set.json', data_folder=args.data_folder)
    preprocess_db(db_paths)
    dh = DataPreprocessor()
    data = dh.preprocess_main()
    if not os.path.exists(f'{args.data_folder}/multi-woz-processed'):
        os.makedirs(f'{args.data_folder}/multi-woz-processed')

    with open(f'{args.data_folder}/multi-woz-processed/data_for_damd.json', 'w') as f:
