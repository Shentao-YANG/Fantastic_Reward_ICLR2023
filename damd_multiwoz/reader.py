import numpy as np
import os, csv, random, logging, json, re
...
class MultiWozReader(_ReaderBase):
    ...
    def _construct_bspn_constraint(self):
        ...
        with open(f'{cfg.data_folder}/multi-woz-processed/bspn_masks.txt', 'w') as f:
    ...
    def _construct_aspn_constraint(self):
        ...
        with open(f'{cfg.data_folder}/multi-woz-processed/aspn_masks.txt', 'w') as f:
        ...
        for fn, dial in self.data.items():
            ...
            if 'all' in cfg.exp_domains or self.exp_files.get(fn):
                if self.dev_files.get(fn):
                    ...
                    # train_count+=1
        if save_temp:
            json.dump(self.test, open(f'{cfg.data_folder}/multi-woz-analysis/test.encoded.json','w'), indent=2)
            self.vocab.save_vocab(f'{cfg.data_folder}/multi-woz-analysis/vocab_temp')
