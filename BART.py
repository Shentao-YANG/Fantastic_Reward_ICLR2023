...
class MiniBART(BartModel):
    ...
    def set_caspi_wt(self, caspi_wt):
        self.caspi_wt = caspi_wt
    ...
    def forward(
        ...
        if not generation_mode:
            # decoder_input_ids, decoder_padding_mask: (batch_size, seq_len); causal_mask: (seq_len, seq_len)
        ...
        # lm_logits: (batch_size, seq_len, vocab_size), e.g., (8, 28, 50333) for 8 sentences
        lm_logits = F.linear(decoder_outputs[0], self.shared.weight)
        outputs = (lm_logits,) + outputs[1:]  # Add hidden states and attention if they are here
        if lm_labels is not None:
            if adopt_caspi==True:
                loss_fct = nn.CrossEntropyLoss(reduction='none')
                # lm_labels: (batch_size, seq_len), e.g. (8, 28); masked_lm_loss: (batch_size * seq_len), e.g. (224=8*28)
                masked_lm_loss = loss_fct(lm_logits.view(-1, self.config.vocab_size), lm_labels.view(-1))
                
                mask = lm_labels>1e-7   # False if the label is pad_token (-100), True o.w.
                mask = mask*1.
                # returns: list with len batch_size -> torch.tensor([batch_size, 1])
                returns = cuda_(torch.from_numpy(np.array(returns)).unsqueeze(-1))
                batch_seq_ones = cuda_(torch.ones(lm_labels.shape))
                returns = returns * batch_seq_ones
                # returns: (batch_size, seq_len) -> (batch_size * seq_len)
                returns = returns.view(-1) 
                masked_lm_loss = masked_lm_loss * returns
                # "/torch.sum(mask)": average loss per non pad_token (for pad_token, the loss is 0)
                masked_lm_loss = self.caspi_wt * torch.sum(masked_lm_loss)/torch.sum(mask)
